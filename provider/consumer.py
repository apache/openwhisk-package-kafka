...
# HEADS UP! I'm importing confluent_kafka.Consumer as KafkaConsumer to
# avoid a naming conflict with my own Consumer class
...
class Consumer:
    ...
    def restart(self):
        ...
        # user may have interleaved a request to delete the trigger,
        # check again
        if self.thread.desiredState() is not Consumer.State.Dead:
            logging.info("[{}] Starting new consumer "
                         "thread".format(self.trigger))
...
class ConsumerThread (Thread):
    ...
    def __init__(self, trigger, params):
        ...
        # always init consumer to None in case the consumer needs to shut
        # down before the KafkaConsumer is fully initialized/assigned
        self.consumer = None

    # this only records the current state, and does not affect a
    # state transition
    ...
    def setDesiredState(self, newState):
        logging.info("[{}] Request to set desiredState "
                     "to {}".format(self.trigger, newState))

        with self.lock:
            if (self.__desiredState is Consumer.State.Dead and
               newState is not Consumer.State.Dead):
                logging.info("[{}] Asking to kill a consumer that "
                             "is already marked for death. Doing "
                             "nothing.".format(self.trigger))
                return
            else:
                logging.info("[{}] Setting desiredState "
                             "to: {}".format(self.trigger, newState))
    ...
    def run(self):
        try:
            ...
            logging.info("[{}] Consumer exiting main "
                         "loop".format(self.trigger))
        except Exception as e:
            logging.error("[{}] Uncaught exception: "
                          "{}".format(self.trigger, e))

        if self.desiredState() == Consumer.State.Dead:
            logging.info("[{}] Permanently killing consumer because "
                         "desired state is Dead".format(self.trigger))
            self.database.deleteTrigger(self.trigger)
        elif self.desiredState() == Consumer.State.Restart:
            logging.info("[{}] Quietly letting the consumer thread stop "
                         "in order to allow restart.".format(self.trigger))
            # nothing else to do because this Thread is about to go away
        else:
            # uh-oh... this really shouldn't happen
            logging.error("[{}] Consumer stopped without being "
                          "asked".format(self.trigger))
        ...
        try:
            ...
            logging.error("[{}] Uncaught exception while shutting down "
                          "consumer: {}".format(self.trigger, e))
        finally:
            logging.info("[{}] Recording consumer as Dead. "
                         "Bye bye!".format(self.trigger))
            ...
            while batchMessages and (self.secondsSinceLastPoll() < 2):
                ...
                if (message is not None):
                    if not message.error():
                        logging.debug("Consumed message"
                                      ": {}".format(str(message)))
                        ...
                        logging.error(
                            "[{}] Error polling: "
                            "{}".format(self.trigger, message.error()))
                        batchMessages = False
                    else:
                        logging.debug(
                            "[{}] No more messages. Stopping "
                            "batch op.".format(self.trigger))
                        batchMessages = False
                else:
                    logging.debug("[{}] message was None. "
                                  "Stopping batch op.".format(self.trigger))
                    batchMessages = False

        logging.debug("[{}] Completed poll".format(self.trigger))

        if len(messages) > 0:
            logging.info("[{}] Found {} messages with a total "
                         "size of {} bytes".format(self.trigger,
                                                   len(messages),
                                                   messageSize))
    ...
    def __fireTrigger(self, messages):
        if self.__shouldRun():
            ...
            logging.info("[{}] Firing trigger with {} "
                         "messages".format(self.trigger, len(messages)))

            while retry:
                try:
                    response = requests.post(self.triggerURL,
                                             json=payload,
                                             timeout=10.0,
                                             verify=check_ssl)
                    status_code = response.status_code
                    logging.info("[{}] Response status "
                                 "code {}".format(self.trigger,
                                                  status_code))

                    # Manually commit offset if the trigger was fired
                    # successfully. Retry firing the trigger if the
                    # service was unavailable (503), an internal server
                    # error occurred (500), the request timed out
                    # (408), or the request was throttled (429).
                    if status_code == 200:
                        logging.info(
                            "[{}] Fired trigger with activation "
                            "{}".format(self.trigger,
                                        response.json()['activationId']))
                        self.consumer.commit()
                        retry = False
                    elif status_code not in [503, 500, 408, 429]:
                        logging.error("[{}] Error talking to OpenWhisk, "
                                      "status code {}".format(self.trigger,
                                                              status_code))
                ...
                if retry:
                    ...
                    if retry_count < self.max_retries:
                        logging.info("[{}] Retrying in {} "
                                     "second(s)".format(self.trigger,
                                                        self.retry_timeout))
                        time.sleep(self.retry_timeout)
                    else:
                        logging.warn("[{}] Skipping {} messages to "
                                     "offset {} of partition "
                                     "{}".format(self.trigger,
                                                 len(messages),
                                                 lastMessage.offset(),
    ...
    def shutdown(self):
        if(self.currentState() != Consumer.State.Stopping and
           self.currentState() != Consumer.State.Dead):
            logging.info("[{}] Shutting down consumer for "
                         "trigger".format(self.trigger))
            self.__recordState(Consumer.State.Stopping)
            self.setDesiredState(Consumer.State.Dead)
        else:
            logging.info("[{}] Ignoring request to shutdown consumer "
                         "for trigger as it is already "
                         "shutting down".format(self.trigger))

    def __encodeMessageIfNeeded(self, value):
        if self.encodeValueAsJSON:
            try:
                parsed = json.loads(value)
                logging.debug("[{}] Successfully encoded a message "
                              "as JSON.".format(self.trigger))
                return parsed
            except ValueError:
                # no big deal, just return the original value
                logging.warn("[{}] I was asked to encode a message as "
                             "JSON, but I failed.".format(self.trigger))
                pass
        ...
        logging.debug("[{}] Returning un-encoded "
                      "message".format(self.trigger))
        return value
